...
class Code128 extends BarcodeBase
{
	...
	private function resolveSubtype()
	{
		if ($this->type == self::TYPE_AUTO)
		{
			...
			else if((strtoupper($this->data) == $this->data) && (!preg_match("/|/", $this->data)))
			{
	 ...
	 * @return void
	 */
	public function draw($fname,$location)
	{
		$this->resolveSubtype();
		...
		$numBarsRequired = ($this->type != self::TYPE_C) ? ((sizeof($charAry)+3) * 11) + 35 : ((sizeof($charAry)/2) * 11) + 35;
		$pxPerBar = (int) ($this->x / $numBarsRequired)-10;
                ...
                foreach($charAry as $k => $char)
		{
			$code = $this->getBar($char);
	                if ($char == '') {
	                    $codeswitch = '11110100010';	
                            #error_log("in code => ".$char." ".$codeswitch."\n",3,"/tmp/mylog.txt");	
                 	    foreach(str_split((string) $codeswitch) as $bit)
		  	    {
				imagefilledrectangle($this->img, $currentX, 0, ($currentX + $pxPerBar), ($this->y - 1), (($bit == '1') ? $black : $white));
				$currentX += $pxPerBar;
			    }
                            $checkSumCollector += 98 * $pos ; // $k will be 0 for our firstk
                 	    $pos++; 
                            #error_log("in code => ".$char." ".$code."\n",3,"/tmp/mylog.txt");	
	                    $codeCR = '11110111010';	
                            foreach(str_split((string) $codeCR) as $bit)
		  	    {
				imagefilledrectangle($this->img, $currentX, 0, ($currentX + $pxPerBar), ($this->y - 1), (($bit == '1') ? $black : $white));
				$currentX += $pxPerBar;
			    }
                            $checkSumCollector += 77 * $pos ; // $k will be 0 for our firstk
                            $pos++; 
                        } 
                        else
                        { 
                             #error_log("code => ".$char." ".$code."\n",3,"/tmp/mylog.txt");	
                             $checkSumCollector += $this->getKey($char) * $pos; // $k will be 0 for our first
                             $pos = $pos + 1;
		...
		foreach($ending as $code)
		{
			foreach(str_split((string) $code) as $bit)
			{
                ...
                imagejpeg($this->img,$this->location."/".$this->fname.".jpg",100);
