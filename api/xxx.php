...
$storename = array ("7-11 (มีค่าบริการเติมเงิน)","TWZ");
	...
	#$RowsCheck = $DatabaseClass->DBNumRows($ResultCheck);
		...
		if(preg_match($pattern, $param_pass) || strlen ($param_pass) > 7 ) {          
			...
			if ($haveimei=="0") {
				...
				if ($imeicheckoth) {
					$msg = "ไม่สามารถลงทะเบียนได้ กรุณา ติดต่อกลับเจ้าหน้าที่";
                                         $dataarray = array("result"=>"FAIL","result_desc"=>$String->tis2utf8($msg));
				}else{
					$xcode = "OTH";
	                                error_log("register IPPS => ".$xcode."\n",3,"/tmp/mylog.txt");	
					$mcode = $member->runid ("01");
					$SqlAdd = $member->creatmember($keyid,$String->sqlEscape($param_bank),$String->sqlEscape($param_bankcode),$param_pincode,$xcode,$mcode,$param_cardid,$bdate,MD5($param_pass,$String->sqlEscape($param_saleid),$String->sqlEscape($param_dcode),$String->sqlEscape($param_fname),$String->sqlEscape($param_address),$String->sqlEscape($param_province),$String->sqlEscape($param_mobileno),$String->sqlEscape($param_imeino),$String->sqlEscape(strtolower($param_email)),$String->sqlEscape($param_type),$String->sqlEscape($param_brand),$String->sqlEscape($param_model),$personal);
					...
					$mailbody = "เรียน คุณ ".$param_fname."<br/>";
					$mailbody .= "&nbsp;&nbsp;ขอบคุณที่เลือกใช้บริการ Easy Card และเพื่อการใช้งาน Easy Card ของท่าน<br/> ให้ทำการคลิกที่ เปิดใช้งาน Easy Card เพื่อทำให้ Easy Card ของท่านสามารถใช้งาน ได้<br/><br/> <a href=\"https://www.easycard.club/activate.php?i=".$keyid."\" target=\"_blank\">เปิดใช้งาน Easy Card</a><br/><br/>(หลังจากทำการเปิดใช้งาน Easy Card แล้ว ท่านสามารถ Log in เข้าสู่บริการได้)<br/>ขอบคุณที่ใช้บริการ Easy Card<br/><br/>";
					...
					$headers .= 'From: ฝ่ายดูแลสมาชิก Easy Card <service-member@easycard.club>' . "\r\n";
                                        ...
                                        */
                                        ...
                                        if ($result_array['status'] == '01') {
                                            ...
                                            $reqstr = $ipps->direct_customer_register_confirm($param_email,$mcode,$param_mobileno,$fname,$lname,$param_cardid);
				        	...
				        	$dataarray = array("result"=>"OK","result_desc"=>$String->tis2utf8($msg));
				                ...
				                $error_mesg=$payment->geterrormesg('direct_register.php','IPPS',$result_array['status']);
                                                $payment->updatememberstatusfail($mcode);
				...
				#$res=mail($param_email, $Subject, $mailbody, $headers);
                         */       
                                ...
                                if ($result_array['status'] == '01') {
                                    ...
                                    $reqstr = $ipps->direct_customer_register_confirm($param_email,$mcode,$fname,$lname,$param_cardid);
                                    $urlstr = "https://www.payforu.com/WebService/payforuservice.svc/APIDirectRegisterRequest";
                                    $module_name_detail=$modulename.":direct_customer_register_confirm";
                                    $resultlogging = $payment->insertlog('',$param_email,'BE:REQ',$module_name_detail,$String->utf82tis($reqstr)); //Logging
                                    $resstr = $ipps->ipps_submit_req($urlstr,$reqstr);
