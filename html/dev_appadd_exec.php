...
$SqlAdd = "insert into tbl_product (p_partnerid,p_code,p_date,p_title,p_categorie,p_detail,p_whatnew,p_version,p_price,p_clipurl,p_url,p_type,p_status) values ('".$_SESSION['DVID']."','".addslashes($param_code)."',now(),'".addslashes($param_title)."','".addslashes($param_categorie)."','".addslashes($param_detail)."','".addslashes($param_whatnew)."','".addslashes($param_vers)."','".addslashes($param_price)."','".$urlcode."','".$param_url."','".addslashes($param_type)."','0')";
...
$Web->AlertWinGo("เพิ่มข้อมูลเรียบร้อย ระบบจะนำท่านไปขั้นตอนต่อไป.","dev_appadd_step2.php?id=" . $id);
die();
?>