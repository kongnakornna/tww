...
Class partner extends MySqlDB {
	function login($username='',$password='') {
		$userarray = array();
        $SqlData = "select * from tbl_partner where p_email='".trim($username)."' and p_password=MD5('".trim($password)."') and p_status='1' order by p_id limit 0,1";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 $SqlUpdate = "update tbl_partner set p_lastlogin=now() where p_email='".trim($username)."'";
