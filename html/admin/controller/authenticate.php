...
Class authenticate extends MySqlDB {
	function login($username='',$password='') {
		$userarray = array();
        $SqlData = "select * from tbl_username where u_username='".trim($username)."' and u_password=MD5('".trim($password)."') and u_status='1' and u_group in ('U','M') order by u_id limit 0,1";
