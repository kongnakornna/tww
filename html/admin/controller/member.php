...
Class member extends MySqlDB {
   ...
   function createmember($keyid='',$param_bank='',$param_bankcode='',$param_pincode='',$xcode='',$mcode='',$param_cardid='',$bdate='',$param_pass='',$param_saleid='',$param_dcode='',$param_fname='',$param_address='',$param_provice='',$param_mobileno='',$param_imeino='',$param_email='',$param_type='',$param_brand='',$param_model='',$personal='' ) {
    ...
    function getdcodefromemail($value='') {
		$RowData = array();
		$dbname = "";
	    $SqlData = "select * from tbl_member where m_email='".$value."' order by m_id";
		$ResultData = $this->DataExecute($SqlData);
		...
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
				 $RowData = $this->DBfetch_array($ResultData,$t);
				 $dbname = $RowData['m_dcode'];
			 }
		}
		return $dbname;
	}
    
    function getmemberfromdcode_u($value='') {
		$RowData = array();
		$dbname = "";
	    $SqlData = "select * from tbl_username where u_code='".$value."'";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
				 $RowData = $this->DBfetch_array($ResultData,$t);
				 $dbname = $RowData['member_id'];
			 }
		}
		return $dbname;
	}
  ...
  function getbankfromemail($value='') {
    ...
    function getallmemberdcode() {
		$RowData = array();
		$dbname = "";
	        $Myarray = "";
                $SqlData = "select * from tbl_member where m_codetype = 'D'  order by m_id";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
				 $RowData = $this->DBfetch_array($ResultData,$t);
				 $Myarray[$RowData['m_email']] = 1;
			 }
		}
		return $Myarray;
	}
    ...
    function getemailbyname($value='') {
		$RowData = array();
	        $SqlData = "select * from tbl_member where m_fullname =  '".$value."' and m_status='1' order by m_id";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
    ...
    function getpaymentcard($value='') {
		...
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
				 ...
				 $bcodelist .= "<tr><td width=\"30%\" class=\"tabletopic\" align=\"right\">¥Ÿ√À— ‡µ‘¡‡ß‘π ".$cardtitle."</td>";
