...
Class payment extends MySqlDB {
	...
	function getpaymentwait_cancel($startdate='',$enddate='') {
		$RowData = array();
		$SqlData = "select * from tbl_payment where p_type = 'I' and p_adddate >'".$startdate."' and p_adddate<='".$enddate."' and p_status='0' and (p_ref1 is not null or p_txnid_wallet is not null) order by p_id";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
        ...
        function getpayment_all($startdate='',$enddate='') {
		$RowData = array();
		$MyData = array();
		$SqlData = "select * from tbl_payment where p_type = 'I' and p_adddate >'".$startdate."' and p_adddate<='".$enddate."'";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
				 $RowData = $this->DBfetch_array($ResultData,$t);
                                 $MyData[$RowData['p_ref']] = 1;	
    	                  }
		}
		return $MyData;
	}
	
       function gettranlog_cancel($startdate='',$enddate='') {
		$RowData = array();
		$SqlData = "select * from tbl_trans_logging where module = 'ewalletpayrequest.php[IPPS]:ewallet_deduct_request' and type = 'BE:RES' and logtime > '".$startdate."' and logtime <= '".$enddate."'";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
        ...
        function getpaymentdatabyrefcode_nodiscount($refcode='') {
		$RowData = array();
		$SqlData = "select * from tbl_payment where p_ref='".$refcode."' and p_type = 'I' order by p_id";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
