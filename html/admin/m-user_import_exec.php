...
if ($_FILES['image1']['name']!='') {
    ...
    if ($uploadresult!='ERROR') {
		...
		while (!feof($fp)) {
		   ...
		   $mline=fgets($fp,4096);
		   if (strlen($mline) > 5) {
			  list ($title,$code,$address,$province,$postcode,$phone,$bankname,$bankcode) = explode (",",$mline);
		      $newcode = splitline($code);
                      #error_log("my code => ".$code."\n",3,"/tmp/mylog.txt");
			  if ($no>1) {
	              $SqlAdd .= ",('M','".$String->sqlEscape($newcode)."',MD5('".$newcode."'),'".$String->sqlEscape($title)."','".$String->sqlEscape($code)."','".$String->sqlEscape($title)."','".$String->sqlEscape($address)."','".$String->sqlEscape($province)."','".$String->sqlEscape($postcode)."','".$String->sqlEscape($bankname)."','".$String->sqlEscape($bankcode)."','".$String->sqlEscape($phone)."','1')";
			  }else{
	              $SqlAdd .= "('M','".$String->sqlEscape($newcode)."',MD5('".$newcode."'),'".$String->sqlEscape($title)."','".$String->sqlEscape($code)."','".$String->sqlEscape($title)."','".$String->sqlEscape($address)."','".$String->sqlEscape($province)."','".$String->sqlEscape($postcode)."','".$String->sqlEscape($bankname)."','".$String->sqlEscape($bankcode)."','".$String->sqlEscape($phone)."','1')";
			  }
		   }
