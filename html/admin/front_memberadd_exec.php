...
$param_passcheck = (!empty($_REQUEST["passcheck"])) ? $_REQUEST["passcheck"] : "";
...
if ($param_passcheck=='') {
	...
	$Web->AlertWinGo("กรุณาตรวจสอบหมายเลข IMEI ก่อนทำการบันทึกค่ะ","front_memberadd_form.php");
}else{
	...
	if ($RowsCheck>0) {
		$DatabaseClass->DBClose();
		$Web->AlertWinGo("อีเมล์นี้มีในระบบแล้ว กรุณาใช้อีเมล์อื่น","front_memberadd_form.php");
	}else{
		if ($payment->getcardno($param_paymentcardno) > 0) {
			$DatabaseClass->DBClose();
			$Web->AlertWinGo("รหัสบัตรชำระเงินถูกนำไปใช้แล้ว","front_memberadd_form.php");
		}else{
            $mcode = $member->runid ('TWZ');

			$SqlAdd = "insert into tbl_member (m_type,m_code,m_registerdate,m_fullname,m_address1,m_address2,m_province,m_postcode,m_mobile,m_imei,m_email,m_status,m_saleid,m_addby,m_adddate,m_producttype,m_productbrand,m_productmodel) values ('TWZ','".$mcode."',now(),'".$String->sqlEscape($param_fname)."','".$String->sqlEscape($param_address1)."','".$String->sqlEscape($param_address2)."','".$String->sqlEscape($param_province)."','".$String->sqlEscape($param_postcode)."','".$String->sqlEscape($param_mobileno)."','".$String->sqlEscape($param_imeino)."','".$String->sqlEscape($param_email)."','1','".$String->sqlEscape($param_saleid)."','".$_SESSION['TWZUsername']."',now(),'".$String->sqlEscape($param_type)."','".$String->sqlEscape($param_brand)."','".$String->sqlEscape($param_model)."')";
