...
$dataList = "รายงานชำระค่าบริการ\n\n";
$dataList .= "วันที่ : ".$period."\n";
$dataList .= "#,รหัสสมาชิก,ชื่อสมาชิก,อีเมล์สมาชิก,เบอร์โทรศัพท์,วันที่ทำรายการ,ชื่อร้านค้า,รายการสินค้า,เบอร์ที่เติม,ยอดรวม,ค่าบริการ,ยอดรวมทั้งสิ้น\n";
...
if ($RowsCheck>0) {
	...
	for ($t=0;$t<$RowsCheck;$t++) {
         ...
         $dataList .= $iNum.".,".$custcode.",".$custname.",".$db_email.",".$custmobile.",".$db_date.",".$partnername.",".$db_detail.",".$db_msisdn.",".number_format($db_price,2,'.',',').",".number_format($db_charge,2,'.',',').",".number_format($db_total,2,'.',',')."\n";
	}
}
