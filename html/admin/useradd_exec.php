...
if ($RowsCheck>0) {
	...
	$SqlAdd = "insert into tbl_username (u_group,u_fullname,u_username,u_password,u_email,u_frontside,u_permission,u_status) values ('U','".addslashes($param_fname)."','".addslashes($param_uname)."',MD5('".$param_pname."'),'".addslashes($param_email)."','N','".addslashes($PermissionList)."','1')";
    $ResultAdd = $DatabaseClass->DataExecute($SqlAdd);

	$DatabaseClass->DBClose();
    $Web->AlertWinGo("เพิ่มข้อมูลเรียบร้อย","user_view.php");
	die();
}
?>