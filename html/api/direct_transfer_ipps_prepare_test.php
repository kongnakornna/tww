...
$modulename='direct_transfer_ipps_prepare_test.php[IPPS]';
...
if ($param_memberid=='' || $param_price=='' || $param_code=='' || $param_email=='') {
	...
	$havemember = $member->havemember($param_email);
        ...
        if ($result_array['account_status'] == 'Active' && ($result_array['result'] == '01' || $result_array['result'] == '00'))
        {
            ...
            $urlstr = "https://test.payforu.com/webService/payforuservice.svc/TransferToMember";
            ...
            $result_array = $ipps->extractresult($resstr);
                 if ($result_array['result'] == '01')  // Status success 
                 {
                       ...
                       $resultpayment = $payment->insert($partnercode,$param_code,$servicename,'I',$param_price,$result_array['fee'],$totalprice,$refcode,$result_array['txnid'],"",'N',$param_email,$param_email,'','','0');
                       $discount = $eservice->getediscount_transfer($param_memberid,$totalprice); 
                       $dataarray = array("result"=>"OK","result_desc"=>"","refcode"=>$refcode,"txnid"=>$result_array['txnid'],"price"=>$param_price,"sercharge"=>$result_array['fee'],"totalprice"=>$totalprice,"otp"=>"","discount"=>$discount['discount_result_own']); 
                 }
                 else
                 {
