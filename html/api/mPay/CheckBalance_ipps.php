...
$modulename='checkBalance.php[MPAY]';
$refkey = $param_refid;
...
elseif (strlen($param_msisdn) <> 10) {
        ...
        if ($mprice <> $mprice) { //Disable for local wallet	
            $msg = "ยอดเงินของท่าน มีจำนวนไม่พอกับราคาสินค้า หรือบริการที่ท่านต้องการ กรุณาเติมเงินเพิ่มด้วยค่ะ";
            $dataarray = array("result"=>"FAIL","result_desc"=>$String->tis2utf8($msg));
        }
        else{ 
           $ch = curl_init(); 
	   $balance_req =  MPAYURL . "?cmd=mcash_balance&msisdn=" . MASTERMOBILE_CHK;
	   ...
	   curl_setopt($ch, CURLOPT_URL, MPAYURL . "?cmd=mcash_balance&msisdn=" . MASTERMOBILE_CHK);
	   ...
	   if ($responsecode=='0') {
		$paymentmpay->insertCheckBalance ($param_email,$actualbalance,$param_req,$refkey,MASTERMOBILE,$data);
                ...
                $resultpayment = $payment->updatepaymentmsisdn($refkey,$param_msisdn); 
