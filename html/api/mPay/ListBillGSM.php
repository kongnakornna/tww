...
elseif (strlen($param_msisdn) <> 10) {
	$msg = 'หมายเลขโทรศัพท์มือถือไม่ถูกต้อง';
	$dataarray = array("result"=>"FAIL","result_desc"=>$String->tis2utf8($msg));
}
else{
   ...
   // if ($ans) {
		...
		$invlist = base64_decode((string) $xml->invoiceList);
                ...
                foreach ($xmlinvlist->array->void as $xmlinv) {
                        ...
                        $temp_array=array("billType"=>trim($property_billtype),"seqNum"=>trim($property_seqnum),"amount"=>trim($property_amount),"sessionId"=>trim($property_sessionid),"overDueFee"=>trim($property_overdue),"billDate"=>trim($property_billdate),"billAccount"=>trim($property_billingAccount),"billData"=>trim($property_billdata),"isPay"=>trim($property_isPay));
                ...
                */
                #error_log("TOPUPGSM1:array => ".print_r($InvoiceValue,true)."\n",3,"/tmp/mylog.txt"); 
               	...
               	if ($status=='' || $status=='error') {
                        ...
                        $dataarray = array("result"=>"FAIL1","result_desc"=>$detail,"refid"=>"");
		}else{
