...
$payment = new payment();
$modulename = 'withdraw_wallet_test.php';
			...
			$dataarray = array("result"=>"FAIL","result_desc"=>$String->tis2utf8($msg));
                     ...
                     $ch = curl_init();
                      ...
                      if ($new_array['status'] == '01')
                      {
                             ...
                             $discount=$eservice->getediscount_withdraw($membercode,$param_price); 
                             $dataarray = array("result"=>"OK","result_desc"=>"","bankcode"=>$bankcode,"bankname"=>$bankname,"price"=>$param_price,"refid"=>$refcode,"txnid"=>$new_array['txnid'],"fee"=>$sercharge,"totalamount"=>$totalprice,"confirmcode"=>$new_array['comfirmc'],"discount"=>$discount['discount_result_own']);
                             $param_code = 'ES9999'; 
                             $servicename = $eservice->gettitle ($param_code);
                             $partnercode = $eservice->getpaymentmap ($param_code);
                             $resultpayment = $payment->insert($partnercode,$param_code,$servicename,'I',$totalprice,$sercharge,$param_price,$refcode,"","",'N',$param_email,$param_email,'','','0'); 
	              }	
	              else{
