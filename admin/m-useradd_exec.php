...
if ($RowsCheck>0) {
	...
	$Web->AlertWinGo("username นี้มีในระบบแล้ว กรุณาใช้ชื่ออื่น","m-user_view.php?keyword=" . $param_uname);
	die();
}else{
	$SqlAdd = "insert into tbl_username (u_group,u_fullname,u_username,u_password,u_email,u_phone,u_shoptitle,u_shopaddress,u_shopprovince,u_shoppostcode,u_bankname,u_bankcode,u_status) values ('M','".$String->sqlEscape($param_fname)."','".$String->sqlEscape($param_uname)."',MD5('".$param_pname."'),'".$String->sqlEscape($param_email)."','".$String->sqlEscape($param_phone)."','".$String->sqlEscape($param_shoptitle)."','".$String->sqlEscape($param_shopaddress)."','".$String->sqlEscape($param_province)."','".$String->sqlEscape($param_postcode)."','".$String->sqlEscape($param_bankname)."','".$String->sqlEscape($param_bankcode)."','1')";
    $ResultAdd = $DatabaseClass->DataExecute($SqlAdd);

	$DatabaseClass->DBClose();
    $Web->AlertWinGo("เพิ่มข้อมูลเรียบร้อย","m-user_view.php");
	die();
}
?>