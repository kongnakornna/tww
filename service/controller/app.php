...
Class app extends MySqlDB {
	...
	function getrecomdatatotal($cat='',$type='',$order='') {
	    ...
	    $SqlData = "select * from tbl_product where p_status='1' and p_recommended='Y' $Criteria1 $Criteria2 $Criteria3";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		return $RowsData;
	}
	...
	function getrecomdata($cat='',$type='',$order='',$min='',$max='') {
	    ...
	    $SqlData = "select * from tbl_product where p_status='1' and p_recommended='Y' $Criteria1 $Criteria2 $Criteria3 limit $min,$max";
		$ResultData = $this->DataExecute($SqlData);
		$RowsData = $this->DBNumRows($ResultData);
		if ($RowsData>0) {
			 for ($t=0;$t<$RowsData;$t++) {
